...
public class LoginAction extends ActionSupport {
    ...
    resourceDetails.setAccessTokenUri("http://localhost:8090/oauth/token"); //user-service
    //resourceDetails.setAuthenticationScheme(AuthenticationScheme.header);
    resourceDetails.setClientId("messaging-client");
    resourceDetails.setClientSecret("secret");
    resourceDetails.setGrantType("password");
    resourceDetails.setScope(Arrays.asList("message.read", "message.write"));
    OAuth2RestTemplate restTemplate = new OAuth2RestTemplate(resourceDetails, new DefaultOAuth2ClientContext());
    ResponseEntity<User> response = restTemplate.getForEntity("http://localhost:8069/login?username={username}&password={password}", User.class, getUsername(), getPassword());
